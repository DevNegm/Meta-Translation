...
const Services = ({data}:any) => {
    ...
    return (
        <motion.section variants={containerVariants} initial="hidden" whileInView="show" id='services' className='max-w-350 w-[90%] mx-auto flex flex-col gap-10'>
            ...
            <motion.div className='grid lg:grid-cols-4 md:grid-cols-3 grid-cols-1 gap-4'>
                {data?.services?.map((service: any, index: number) => (
                    <motion.div variants={containerVariants} initial="hidden" whileInView="show" className='flex flex-col gap-5 p-4 rounded-lg bg-accent' key={index}>
                        <motion.img variants={imageVariants} src={service?.image} className='w-full h-50 object-cover rounded-2xl' alt={service?.[`title_${locale}`]} />
