...
const Service = ({ data }: any) => {
    ...
    return (
        <motion.section variants={containerVariants} initial="hidden" whileInView="show" id='our_service' className='max-w-350 w-[90%] mx-auto flex flex-col items-center gap-20 relative'>
            ...
            <motion.div variants={containerVariants} initial="hidden" whileInView="show" className='w-full flex lg:flex-row flex-col items-center justify-center gap-10 relative'>
                <motion.p variants={itemVariants} className='font-semibold'>{data?.our_service?.[`sub1_${locale}`]}</motion.p>
                <motion.p variants={itemVariants} className='font-semibold'>{data?.our_service?.[`sub2_${locale}`]}</motion.p>
                <motion.p variants={itemVariants} className='font-semibold'>{data?.our_service?.[`sub3_${locale}`]}</motion.p>
            </motion.div>
            <motion.div variants={containerVariants} initial="hidden" whileInView="show" className='w-full flex lg:flex-row flex-col items-center justify-center gap-10 relative'>
                <motion.div variants={containerVariants} initial="hidden" whileInView="show" className='w-full lg:w-fit flex flex-col gap-10'>
                    ...
                    <motion.div variants={containerVariants} initial="hidden" whileInView="show" className='w-full flex flex-col gap-5 p-4 py-6 rounded-lg bg-accent border-s-5 border-zinc-600'>
                        <motion.h3 variants={itemVariants} className='lg:text-xl text-base font-semibold'>{data?.our_service?.[`note_desc_${locale}`]}</motion.h3>
